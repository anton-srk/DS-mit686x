...
val_texts, val_labels = zip(*((sample['text'],
                               sample['sentiment'])
                            for sample in val_data))
test_texts, test_labels = zip(*((sample['text'],
                                 sample['sentiment']) for sample in test_data))
...
#    tmp = p1.perceptron(toy_features, toy_labels, t-1)[0]
#    tmp1 = p1.perceptron(toy_features, toy_labels, t)[0]
#
#    if abs(np.dot((tmp1-tmp).T, tmp1)) <= 0.001:
#        print(tmp1, tmp)
#        break

#for t in range(2, 2000, 100):
#    tmp = p1.average_perceptron(toy_features, toy_labels, t-1)[0]
#    tmp1 = p1.average_perceptron(toy_features, toy_labels, t)[0]
#    if abs(np.dot((tmp1-tmp).T, tmp1)) <= 0.001:
#        print(tmp1, tmp)
#        break

#for t in range(2, 2000, 100):
#    tmp = p1.pegasos(toy_features, toy_labels, t-1, L)[0]
#    tmp1 = p1.pegasos(toy_features, toy_labels, t, L)[0]
#    if abs(np.dot((tmp1-tmp).T, tmp1)) <= 0.001:
#        print(tmp1, tmp)
#        break
...
#        np.max(pct_tune_results[1]),
#        Ts[np.argmax(pct_tune_results[1])]))
#
#avg_pct_tune_results = utils.tune_avg_perceptron(Ts, *data)
#print('avg perceptron valid:', list(zip(Ts, avg_pct_tune_results[1])))
#print('best = {:.4f}, T={:.4f}'.format(
#        np.max(avg_pct_tune_results[1]),
...
print(p1.classifier_accuracy(p1.pegasos, train_bow_features,
                             test_bow_features, train_labels,
                             test_labels, T=25, L=0.01))

th, th0 = p1.pegasos(train_bow_features, train_labels, T=25, L=0.01)
# -----------------------------------------------------------------------------
