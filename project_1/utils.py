...
def load_toy_data(path_toy_data):
    ...
    Returns the tuple (features, labels) in which features
    is an Nx2 numpy matrix and
...
def load_data(path_data, extras=False):
    ...
    * text: the text of the review
    ...
    Additionally, if the `extras` argument is True,
    each dict will also include the
...
def write_predictions(path_submit_data, preds):
    ...
    assert len(preds) == len(data), 'Expected {} predictions but {}\
    were given.'.format(len(data), len(preds))
...
def plot_toy_data(algo_name, features, labels, thetas):
    ...
    * thetas - the tuple (theta, theta_0) that is the output
...
def tune(train_fn, param_vals, train_feats,
...
def most_explanatory_word(theta, wordlist):
    """Returns the word associated with the bag-of-words
    feature having largest weight."""
