...
# --Simple quests
quests = ['You are bored.', 'You are getting fat.',
          'You are hungry.', 'You are sleepy.']
quests_map = {}
...
# aligned to quests above
quest_actions = ['watch', 'exercise', 'eat', 'sleep']
# aligned to quest actions above
quest_objects = ['tv', 'bike', 'apple', 'bed']
...
rooms_desc = {'Living': living_desc, 'Garden': garden_desc,
              'Kitchen': kitchen_desc, 'Bedroom': bedroom_desc}
rooms_desc_map = {}
...
rooms_transit = {'Living': living_transit, 'Garden': garden_transit,
                 'Kitchen': kitchen_transit, 'Bedroom': bedroom_transit}
...
# build a map rooms_desc_map that maps a room description
# to the corresponding room index.
...
def load_game_data():
    ...
    for room_name in rooms_valid_acts:
        ...
        for valid_index, act in enumerate(valid_acts):
            ...
            # valid commands: A(h,(a,o))=1 if (a,o) is valid
            # for hidden state h.
            ...
            transit_matrix[room_index, act_index,
                           obj_index, next_room_index] = 1

    text_to_hidden_state_mapping()


# take a step in the game
def step_game(current_room_desc, current_quest_desc,
    ...
    if (command_is_valid[current_room_index, action_index, object_index] == 1):
        ...
        next_room_dist = transit_matrix[current_room_index,
                                        action_index, object_index, :]
        next_room_index = np.random.choice(NUM_ROOMS, p=next_room_dist)
        next_room_name = rooms[next_room_index]
        next_room_desc_index = np.random.randint(
                                            len(rooms_desc[next_room_name]))
